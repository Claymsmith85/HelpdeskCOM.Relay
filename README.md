...
- On `tickets.update`, with **`SUBMITTER_REPLIES` on**, emails the requester only when the last event **itself carries** an agent-authored message that is not sourced from email, not private, not a `System note:`, and not blank — and sends that event's own text. Non-message events (status changes, reassignments, follower/loop-list edits, attachments-only replies) send the requester nothing: the payload's `events` array is the ticket's full history, so falling back to an older visible message would echo the customer's own last message back to them on every agent UI action.
