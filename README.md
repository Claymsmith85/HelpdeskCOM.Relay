...
- On UI-authored `tickets.create` events, patches `customFields.email` with the requester address whenever the webhook is not dark. With **`SUBMITTER_REPLIES` on**, emails the requester only when the create's last event is an agent-authored public message. Customer-emails-in (client-authored) are already handled by `process-mail`, so they are **not echoed back**.
- On `tickets.update`, with **`SUBMITTER_REPLIES` on**, emails the requester only when the webhook's own action carries an agent-authored message that is not sourced from email, not private, not a `System note:`, and not blank — and sends that event's own text. Normally that message is the last event. The one exception: an agent reply that also auto-assigns the ticket (or auto-changes its status) lands trailing assignment/status events **after** the message in the same action, so the selection skips a trailing run of message-less assignment/status companions and accepts the message behind them, but **only** when its timestamp is within seconds of the last event (same-action proof) — without this, a reply on an unassigned ticket was silently dropped until the agent assigned themselves and resent. Standalone non-message events (status changes, reassignments, follower/loop-list edits, attachments-only replies) still send the requester nothing: the payload's `events` array is the ticket's full history, so falling back to an older visible message would echo the customer's own last message back to them on every agent UI action. A per-`(ticket, event)` create-once claim (`reply-claims.ts`, same storage account as the drain locks) makes each message event's requester email send-once across webhook deliveries; the claim is availability-first (a storage failure logs and still sends — losing a customer reply is worse than a rare duplicate).
