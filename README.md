...
src/
  ...
  functions/
    ...
    helpdesk-gate.ts       # Global 429 cooldown: one shared pause instead of per-caller 429s
    ...
    helpdesk-attachments.ts # Webhook attachment prep: direct outbound file attach + link fallback
...
- On UI-authored `tickets.create` events, with **`SUBMITTER_REPLIES` on**, emails the requester only when the create's last event is an agent-authored public message. Customer-emails-in (client-authored) are already handled by `process-mail`, so they are **not echoed back**. The address comes from the ticket's Submitter (`payload.requester.email`).
- On `tickets.update`, with **`SUBMITTER_REPLIES` on**, emails the requester only when the webhook's own action carries an agent-authored message that is not sourced from email, not private, not a `System note:`, and not blank — and sends that event's own text. Normally that message is the last event. The sanctioned same-action exceptions are trailing message-less companion metadata events (assignment/status/attachments) that can land **after** the message in one Helpdesk action, so the selection skips a short trailing run of those companions and accepts the message behind them, but **only** when its timestamp is within seconds of the last event (same-action proof). Standalone non-message events (status changes, reassignments, follower/loop-list edits, standalone attachment edits with no message) still send the requester nothing: the payload's `events` array is the ticket's full history, so falling back to an older visible message would echo the customer's own last message back to them on every agent UI action. A per-`(ticket, event)` create-once claim (`reply-claims.ts`, same storage account as the drain locks) makes each message event's requester email send-once across webhook deliveries; the claim is availability-first (a storage failure logs and still sends — losing a customer reply is worse than a rare duplicate).
- Stage 1 attachment forwarding (webhook mail only): when a selected message action carries attachment files, the relay attempts to attach those files directly to outbound requester/notices emails via Graph `fileAttachment` payloads. Files over the stage-1 inline cap (or files that fail download) are skipped and their message links remain as fallback; successful small-file attaches do not require a separate resend.
...
- **Follower / people-in-the-loop notices** (`FOLLOWERS_NOTICES`, default OFF): on every create/update, independently of the requester gates above, the ticket's followers (`payload.followers`, agents) and people-in-the-loop (`payload.cc`, external emails) are emailed about the webhook's action event — the last event, except that a reply which auto-assigns/auto-changes status notices as the reply rather than its trailing companion marker (the same same-action selection and per-`(ticket, event)` send-once claim as the requester path; standalone reassignments still notice as assignments) — threaded with the `[#shortID]` tag so their replies match back into the ticket. Visibility is unchanged: followers get everything (public messages, private notes, system notes, status changes, assignment changes, follower/loop-list changes); people-in-the-loop get public messages, status changes, and loop-list changes only. Echo control excludes the requester, event author, and `[Relayed from …]` sender for this audience. Every recipient in both notice audiences passes the outbound loop guard. Notice failures are best-effort and never fail the webhook (a non-200 would make Helpdesk redeliver and duplicate email).
  - When every attachment file for that message is attached directly, the relay removes the trailing Helpdesk boilerplate attachment list block from the copied message text (`---` / `Attachments:` / bullet list) to avoid duplicate presentation (inline files + repeated filename list). If any file is skipped, the text remains unchanged so links stay visible.
...
**Requester addresses.** The customer's real email address is stored unaltered as the Helpdesk ticket **Submitter** (requester), and that is the single source the webhook sends agent replies to (`payload.requester.email`). The old `customFields.email` mirror is **retired** — the custom field has been deleted in Helpdesk, and the relay neither writes nor reads it. This is safe **only because Helpdesk's own requester notifications are disabled in Helpdesk's admin settings** — the relay is the sole sender of customer email (reply-received ack for existing tickets, agent replies via the webhook). **Re-enabling Helpdesk's requester notifications would double-email customers on every event.**
...
- **HTTP resilience.** The Graph and Helpdesk axios clients carry a per-request timeout (`GRAPH_HTTP_TIMEOUT_MS` / `HELPDESK_HTTP_TIMEOUT_MS`, default 60 s; attachment transfers use `GRAPH_TRANSFER_TIMEOUT_MS`, default 300 s) so one hung call can't burn the whole invocation, plus a shared retry policy that honors `Retry-After`. Retry safety: `429`/`503` (server rejected before processing) retry for any method; ambiguous `5xx`/transport errors retry **only** for idempotent reads, so a retry never duplicates a ticket or an outbound email. Helpdesk calls are additionally paced at 5 requests/second per Function worker process by default, including retries, and use a Helpdesk-scoped retry ladder of 5 retries with a 60 s maximum delay. The Graph and Storage clients keep the shared retry defaults (3 retries, 500 ms base, 20 s cap) — deliberately modest, because the drain makes several Graph and Storage calls per message inside a 10-minute `functionTimeout`, so one blip should cost seconds rather than minutes. The maximum delay bounds the **jittered** total, so it means what it says. Terminal HTTP errors carry an explicit `api` (`Helpdesk`, `Microsoft Graph`, `Azure Blob Storage`, or `Azure Queue Storage`) and completed retry count in structured logs, and the API name prefixes the error message for unstructured/runtime logs.
- **Global Helpdesk throttle gate.** `http-retry.ts` backs off a single request and holds no state anyone else can see, so a throttled Helpdesk used to be rediscovered independently by every queue item, webhook, and retry — each spending a request to earn its own `429`. `helpdesk-gate.ts` records one shared cooldown from the `Retry-After` (or `HELPDESK_GATE_DEFAULT_COOLDOWN_MS` when absent, capped by `HELPDESK_GATE_MAX_COOLDOWN_MS`) that every subsequent Helpdesk request waits on. It only ever extends, never shortens. Callers that can put work back do so rather than sleep: `process-mail` re-enqueues its drain item and returns **without throwing** (no dequeue count burned, no lease held) and `sync-teams` skips the run, both above `HELPDESK_GATE_DEFER_ABOVE_MS`; the wait spent before re-enqueueing is what stops an immediately-visible queue message from spinning. The `helpdesk` webhook is the exception — it has nowhere to defer to and a slow response makes Helpdesk redeliver (duplicating email), so it waits only `HELPDESK_GATE_WEBHOOK_MAX_WAIT_MS` and then calls anyway. The deadline is process-local and additionally shared via a blob (read once per invocation, written on penalty) so a second instance inherits the pause. **Graph and Storage are deliberately not gated** — their work is unrelated to Helpdesk's rate limit, and freezing it would make a drain more likely to hit its 10-minute timeout mid-message.
- **Helpdesk per-call telemetry.** Every Helpdesk request *attempt* emits one Application Insights entry from an interceptor inside `createHelpdeskClient`, so a throttled or failed call is visible without the caller having to log it. Severity keeps `severityLevel >= 3` a real failure signal: a clean call is info (`Helpdesk API`), a call that succeeded after retries is a warning (`Helpdesk API ok after retries`), each attempt that is about to be retried is a warning (`Helpdesk API retryable failure` — this is where a `429` shows up), and only a terminal failure is an error (`Helpdesk API FAILED`). Entries carry `method`, `path`, `status`, `ms` and `retries`; failures add the response body (capped) and a **shape-only** request-body summary — field names and byte size, never values, so customer email content never reaches App Insights. The handler passes its `createStepLogger` result to `createHelpdeskClient`; omitting it makes the client silent.
...
- Agent replies to the requester (via `helpdesk`, gated by `SUBMITTER_REPLIES`) with stage-1 direct attachment forwarding for eligible small files, and link fallback for skipped files.
...
Webhook outbound attachments (Stage 1):
- For `helpdesk`-driven requester replies and notices, the relay attempts direct Graph file attachments from Helpdesk event file URLs when each file is at or below `OUTBOUND_INLINE_ATTACHMENT_MAX_BYTES` (default 2,500,000 bytes).
- If every file is attached, the trailing Helpdesk `Attachments:` block is stripped from the copied message text.
- If one or more files are too large or fail download, the email still sends and keeps the original text/link block as fallback.
- The same outbound email can still intentionally target multiple audiences (requester vs assigned-agent notices), but attachment companions do not create duplicate sends by themselves because send-once claims remain keyed by message event ID.
...
| `OUTBOUND_INLINE_ATTACHMENT_MAX_BYTES` | Stage-1 webhook outbound direct-attachment cap in bytes (default `2500000`). Files over this cap stay as link fallback in message text. |
...
| `HELPDESK_GATE_ENABLED` | Global Helpdesk throttle gate: a `429` pauses **all** Helpdesk calls until the deadline instead of each caller earning its own (default `true`; set `false` as an escape hatch). Graph and Storage are never gated. |
| `HELPDESK_GATE_DEFAULT_COOLDOWN_MS` | Cooldown applied when a `429` carries no `Retry-After` (default `10000`). |
| `HELPDESK_GATE_MAX_COOLDOWN_MS` | Ceiling on any single cooldown, so a bogus `Retry-After` can't park the relay (default `300000`). |
| `HELPDESK_GATE_DEFER_ABOVE_MS` | Remaining cooldown above which a deferrable caller puts the work back instead of waiting — `process-mail` re-enqueues its drain item, `sync-teams` skips the run (default `30000`). Also the pacing budget spent in-invocation before that re-enqueue. |
| `HELPDESK_GATE_MAX_WAIT_MS` | How long a single request waits on the gate before dispatching anyway (default `30000`). |
| `HELPDESK_GATE_WEBHOOK_MAX_WAIT_MS` | The `helpdesk` webhook's much smaller budget (default `5000`). It cannot defer — a slow response makes Helpdesk redeliver and duplicate email — so it cooperates briefly, then calls regardless. |
| `HELPDESK_GATE_SHARED` | Publish/read the cooldown deadline via a blob so other instances inherit the pause (default `true`). Read once per invocation, written when this process is penalized; best-effort, so a storage failure only falls back to the process-local gate. |
| `HELPDESK_GATE_CONTAINER` | Container for the shared cooldown blob (default `relay-state`, same account/UAMI as the claims — no new RBAC). |
...
| `helpdesk-client.test.ts` | `findTicketByShortId`: shortID param + client-side verification, 4xx→null / 5xx→throw; client wiring (idempotent interceptors) and per-call telemetry severity + request-body redaction |
| `logging.test.ts` | `summarizeRequestBody`: shape-only summaries (field names + size), never values |
| `helpdesk-gate.test.ts` | global 429 cooldown: Retry-After honored/defaulted/capped, extend-only, interceptor wait + budget exhaustion, defer decision |
